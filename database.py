...
if DATABASE_URL.startswith('postgresql'):
    # PostgreSQL/Supabase butuh SSL dan timeout
...
def init_db():
    ...
    try:
        ...
        try:
            ...
            if admin_count == 0:
                # Cek kalau ada environment variables untuk auto-create admin
                admin_username = os.getenv('ADMIN_USERNAME', '').strip()
                admin_password = os.getenv('ADMIN_PASSWORD', '').strip()
                jwt_secret = os.getenv('JWT_SECRET_KEY', '').strip()
                
                if admin_username and admin_password and jwt_secret:
                    # Auto-create admin untuk production deployment
                    try:
                        from admin_auth import hash_password
                        logger.info("=" * 80)
                        logger.info("🔧 Auto-create admin user dari environment variables...")
                        logger.info("=" * 80)
                        ...
                        logger.error(f"❌ Error saat auto-create admin: {e}")
                        db.rollback()
                else:
                    logger.warning("=" * 80)
